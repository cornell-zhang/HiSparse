...
// bit-casted edge payload (COO)
struct BIT_EDGE_PLD_T {
    ap_uint<32> mat_val;
    IDX_T row_idx;
    IDX_T col_idx;
    INST_T inst;
};
#define BIT_EDGE_PLD_SOD ((BIT_EDGE_PLD_T){0,0,0,SOD})
#define BIT_EDGE_PLD_EOD ((BIT_EDGE_PLD_T){0,0,0,EOD})
#define BIT_EDGE_PLD_EOS ((BIT_EDGE_PLD_T){0,0,0,EOS})

// bit-casted update payload
struct BIT_UPDATE_PLD_T {
    ap_uint<32> mat_val;
    ap_uint<32> vec_val;
    IDX_T row_idx;
    INST_T inst;
};
#define BIT_UPDATE_PLD_SOD ((BIT_UPDATE_PLD_T){0,0,0,SOD})
#define BIT_UPDATE_PLD_EOD ((BIT_UPDATE_PLD_T){0,0,0,EOD})
#define BIT_UPDATE_PLD_EOS ((BIT_UPDATE_PLD_T){0,0,0,EOS})

#ifndef __SYNTHESIS__

std::ostream& operator<<(std::ostream& os, const BIT_EDGE_PLD_T &p) {
    os << '{'
        << "mat val: " << p.mat_val << '|'
        << "row idx: " << p.row_idx << '|'
        << "col idx: " << p.col_idx << '|'
...
std::ostream& operator<<(std::ostream& os, const BIT_UPDATE_PLD_T &p) {
    os << '{'
        << "mat val: " << p.mat_val << '|'
        << "vec val: " << p.vec_val << '|'
        << "row idx: " << p.row_idx << '|'
...
//------------------------------------------------------------
// arbiter for BIT_EDGE_PLD_T (depends on col_idx)
template<unsigned num_lanes>
void arbiter_1p(
    const BIT_EDGE_PLD_T in_pld[num_lanes],
    BIT_EDGE_PLD_T resend_pld[num_lanes],
...
// arbiter for BIT_UPDATE_PLD_T (depends on row_idx)
template<unsigned num_lanes>
void arbiter_1p(
    const BIT_UPDATE_PLD_T in_pld[num_lanes],
    BIT_UPDATE_PLD_T resend_pld[num_lanes],
    const ap_uint<num_lanes> in_valid,
    ap_uint<1> in_resend[num_lanes],
    unsigned xbar_sel[num_lanes],
    ap_uint<num_lanes> &out_valid,
...
//------------------------------------------------------------
// shuffler core (2 overloads)
//------------------------------------------------------------
// shuffler core for EDGE_PLD_T
template<unsigned num_lanes>
void shuffler_core(
    // fifos
    hls::stream<EDGE_PLD_T> input_lanes[num_lanes],
    hls::stream<EDGE_PLD_T> output_lanes[num_lanes]
) {
    ...
    // payloads
    EDGE_PLD_T payload_in[num_lanes];
    BIT_EDGE_PLD_T payload[num_lanes];
    // IDX_T in_addr[num_lanes];
    #pragma HLS array_partition variable=payload_in complete
    #pragma HLS array_partition variable=payload complete
    // #pragma HLS array_partition variable=in_addr complete
    ap_uint<num_lanes> valid = 0;

    // resend control
    BIT_EDGE_PLD_T payload_resend[num_lanes];
...
// #endif
        for (unsigned ILid = 0; ILid < num_lanes; ILid++) {
            ...
            if (resend[ILid]) {
                ...
                valid[ILid] = 0;
                payload[ILid] = (BIT_EDGE_PLD_T){0,0,0,0};
            } else {
                if (input_lanes[ILid].read_nb(payload_in[ILid])) {
                    if (payload_in[ILid].inst == EOD) {
                        fetch_complete[ILid] = 1;
                        valid[ILid] = 0;
                    } else {
                        valid[ILid] = 1;
                    }
                    payload[ILid].mat_val = val2bit(payload_in[ILid].mat_val);
                    payload[ILid].row_idx = payload_in[ILid].row_idx;
                    payload[ILid].col_idx = payload_in[ILid].col_idx;
                    payload[ILid].inst = payload_in[ILid].inst;
                } else {
                    valid[ILid] = 0;
                    payload[ILid] = (BIT_EDGE_PLD_T){0,0,0,0};
                }
        ...
        for (unsigned OLid = 0; OLid < num_lanes; OLid++) {
            ...
            if (xbar_valid[OLid]) {
                if (valid[xbar_sel[OLid]]) {
                    EDGE_PLD_T pt;
                    pt.mat_val = bit2val(payload[xbar_sel[OLid]].mat_val);
                    pt.row_idx = payload[xbar_sel[OLid]].row_idx;
                    pt.col_idx = payload[xbar_sel[OLid]].col_idx;
                    pt.inst = payload[xbar_sel[OLid]].inst;
                    output_lanes[OLid].write(pt);
                }
            }
// #ifndef __SYNTHESIS__
//             if (line_tracing_shuffle_core) {
...
// shuffler core for UPDATE_PLD_T
template<unsigned num_lanes>
void shuffler_core(
    // fifos
    hls::stream<UPDATE_PLD_T> input_lanes[num_lanes],
    hls::stream<UPDATE_PLD_T> output_lanes[num_lanes]
) {
    const unsigned shuffler_extra_iters = (ARBITER_LATENCY + 1) * num_lanes;
    // pipeline control variables
    ap_uint<num_lanes> fetch_complete = 0;
    ...
    // payloads
    UPDATE_PLD_T payload_in[num_lanes];
    BIT_UPDATE_PLD_T payload[num_lanes];
    // IDX_T in_addr[num_lanes];
    #pragma HLS array_partition variable=payload_in complete
    #pragma HLS array_partition variable=payload complete
    // #pragma HLS array_partition variable=in_addr complete
    ...
    // resend control
    BIT_UPDATE_PLD_T payload_resend[num_lanes];
    #pragma HLS array_partition variable=payload_resend complete
    ap_uint<1> resend[num_lanes];
    #pragma HLS array_partition variable=resend complete
    for (unsigned i = 0; i < num_lanes; i++) {
...
// #endif
        for (unsigned ILid = 0; ILid < num_lanes; ILid++) {
            ...
            if (resend[ILid]) {
                ...
                valid[ILid] = 0;
                payload[ILid] = (BIT_UPDATE_PLD_T){0,0,0,0};
            } else {
                if (input_lanes[ILid].read_nb(payload_in[ILid])) {
                    if (payload_in[ILid].inst == EOD) {
                        fetch_complete[ILid] = 1;
                    ...
                    payload[ILid].mat_val = val2bit(payload_in[ILid].mat_val);
                    payload[ILid].vec_val = val2bit(payload_in[ILid].vec_val);
                    payload[ILid].row_idx = payload_in[ILid].row_idx;
                    payload[ILid].inst = payload_in[ILid].inst;
                } else {
                    valid[ILid] = 0;
        ...
        for (unsigned OLid = 0; OLid < num_lanes; OLid++) {
            ...
            if (xbar_valid[OLid]) {
                if (valid[xbar_sel[OLid]]) {
                    UPDATE_PLD_T pt;
                    pt.mat_val = bit2val(payload[xbar_sel[OLid]].mat_val);
                    pt.vec_val = bit2val(payload[xbar_sel[OLid]].vec_val);
                    pt.row_idx = payload[xbar_sel[OLid]].row_idx;
                    pt.inst = payload[xbar_sel[OLid]].inst;
                    output_lanes[OLid].write(pt);
                }
...
// #endif
        ...
        shuffler_core<num_lanes>(input_lanes, output_lanes);
// #ifndef __SYNTHESIS__
//         if (line_tracing_shuffle) {
