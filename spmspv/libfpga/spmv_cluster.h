...
static T array_max(T array[len]) {
    #pragma HLS inline
...
static void CPSR_matrix_loader(
...
static void spmv_vector_unpacker (
    hls::stream<VEC_AXIS_INTERNAL_T> &vec_in,
    hls::stream<VEC_PLD_T> vec_out[PACK_SIZE]
) {
    bool exit = false;
    while (!exit) {
        #pragma HLS pipeline II=1
        VEC_AXIS_INTERNAL_T pkt = vec_in.read();
        for (unsigned k = 0; k < PACK_SIZE; k++) {
            #pragma HLS unroll
            VEC_PLD_T p;
...
static void spmv_result_packer (
    ...
    hls::stream<VEC_AXIS_INTERNAL_T> &res_out
) {
    bool exit = false;
    unsigned pkt_idx = 0;
    while (!exit) {
...
static void spmv_cluster(
    const SPMV_MAT_PKT_T *matrix_hbm,       // in
    hls::stream<VEC_AXIS_INTERNAL_T> &vec_in,        // in
    hls::stream<VEC_AXIS_INTERNAL_T> &res_out,       // out
    ...
    std::cout << "INFO : [SpMV cluster] Enter SpMV cluster" << std::endl;
#endif
