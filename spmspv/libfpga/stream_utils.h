...
template<unsigned N>
static void axis_duplicate(
    hls::stream<VEC_AXIS_T> &in,
    hls::stream<VEC_AXIS_INTERNAL_T> out[N]
) {
    bool exit = false;
    while (!exit) {
        #pragma HLS pipeline II=1
        VEC_AXIS_T pkt = in.read();
        VEC_AXIS_INTERNAL_T pkt_out;
        pkt_out.data = pkt.data;
        pkt_out.user = pkt.user;
...
template<unsigned N>
static void axis_merge(
    hls::stream<VEC_AXIS_INTERNAL_T> in[N],
    ...
    while (!exit) {
        #pragma HLS pipeline II=1
        if (!got_EOS[i]) {
            VEC_AXIS_INTERNAL_T pkt = in[i].read();
