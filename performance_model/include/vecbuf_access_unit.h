...
void vecbuf_writer(
    ...
    VAL_T *vector_buffer, // double buffering
    const unsigned bank_size
) {
    bool loop_exit = false;
...
void vecbuf_access_unit(
    ...
    for (unsigned i = 0; i < num_partitions + 1; i++) {
        #pragma HLS dataflow
        vecbuf_writer<id, pack_size>(vec_input, vector_buffer, bank_size);
        vecbuf_reader<id, pack_size>(input, output, vector_buffer, bank_size);
    }
}
