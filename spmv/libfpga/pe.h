...
template<int id, unsigned bank_size, unsigned pack_size>
void ufixed_pe_process_flushable(
    hls::stream<UPDATE_PLD_T> &input,
    VAL_T output_buffer[bank_size],
    IDX_T &flush_size
) {
...
void pe_flushable(
    ...
    while (!exit) {
        #pragma HLS pipeline off
        // this function will exit upon EOD
        ufixed_pe_process_flushable<id, bank_size, pack_size>(input, output_buffer, flush_size);
