...
void spmv_vector_duplicator (
    ...
    while (!exit) {
        ...
        std::cout << "SpMV Vector Duplicator write to output " << k << ": " << p
...
#ifdef SPMV_VECTOR_DUPLICATOR_LINE_TRACING
        std::cout << "SpMV Vector Duplicator write to output " << k << ": " << p
                  << " (special token)" << std::endl;
#endif
            }
            fetch_new = true;
            cnt = 0;
        }
        exit = fetch_new && (pkt.user == EOS);
    }
}
...
void spmv_cluster(
    ...
    #pragma HLS stream variable=DUP2VAU depth=FIFO_DEPTH

    #pragma HLS bind_storage variable=ML2VAU   type=FIFO impl=SRL
    #pragma HLS bind_storage variable=VAU2PE   type=FIFO impl=SRL
    #pragma HLS bind_storage variable=PE2PK   type=FIFO impl=SRL
    #pragma HLS bind_storage variable=DUP2VAU type=FIFO impl=SRL
    ...
    std::cout << "INFO : [SpMV cluster] Vector Duplicator complete" << std::endl;
#endif
